...
function addSeatType() {
    if (seatTypeText.value && promoMontant.value) {
        seatTypeTagify.addTags([
            seatTypeText.value + ' : ' + promoMontant.value + '€',
        ]);
        seatTypeText.value = '';
        promoMontant.value = '';
        seatTypeText.focus();
...
function setColor(placeType) {
    ...
    const rule = Array.from(styleSheet.cssRules).find(
        (r) => r.selectorText === `.seat.${placeType}`
    );
    if (!color) {
            ...
            all_seat.forEach(function (seat) {
                if (seat.classList[0] !== 'select-row') {
                    if (
                        ...
                        const other_classes = [...seat.classList].filter(
                            (c) => c !== 'seat' && c !== 'space'
                        );
                        ...
                        if (selected_type !== '') {
                            if (!seat.classList.contains(selected_type)) {
                                ...
                                if (!rule) {
                                    styleSheet.insertRule(
                                        `.seat.${selected_type} { color: ${color}; }`,
                                        styleSheet.cssRules.length
                                    );
                                } else {
                                    rule.style.color = color;
...
function create_seat_svg() {
    ...
    create_path_svg(
        svg,
        'M9,29H5c-1.1,0-2-0.9-2-2V17c0-1.7,1.3-3,3-3h0c1.7,0,3,1.3,3,3V29z'
    );
    create_path_svg(
        svg,
        'M27,29h-4V17c0-1.7,1.3-3,3-3h0c1.7,0,3,1.3,3,3v10C29,28.1,28.1,29,27,29z'
    );
    create_rect_svg(svg, 9, 19, 14, 10);
    create_rect_svg(svg, 9, 9, 14, 10);
    create_path_svg(
        svg,
        'M6,14V7.8C6,5.7,7.7,4,9.8,4h12.3C24.3,4,26,5.7,26,7.8V14'
    );
    ...
    for (const index in json_dictionnary) {
        if (json_dictionnary[index].class != 'none') {
            ...
            if (row.classList.contains('seat-row')) {
                ...
                const select_row_svg = document.createElementNS(
                    'http://www.w3.org/2000/svg',
                    'svg'
                );
                const select_row_path = document.createElementNS(
                    'http://www.w3.org/2000/svg',
                    'path'
                );
...
function tmp_create() {
    ...
    const new_json = [];
    new_json.push({
        nom: document.querySelector('#id_name').value,
        class: 'none',
        seat_types: types,
    });
    const seat_area = document.querySelector('.seat-area');
    const row = seat_area.childNodes;
    for (let i = 0; i < row.length; i++) {
        if (row[i].classList[0] === 'standing-zone') {
            const nbr_place = document.querySelector(
                '#id_standing_number'
            ).value;
            new_json.push({ class: 'standing-zone', nbr_place });
        } else if (row[i].classList[0] === 'seat-row') {
            const tmp_json = { class: 'seat-row' };
            const seats = row[i].childNodes;
            const array_seat = [];
...
document.querySelector('#create_json').addEventListener('click', (event) => {
    ...
    if (document.querySelector('#option').value === 'nothing') {
        const new_json = tmp_create();
        const csrfTokenValue = document.querySelector(
            '[name=csrfmiddlewaretoken]'
        ).value;
        const request = new Request('/configuration/create_json/', {
            method: 'POST',
            headers: {
...
function set_place_type(seat) {
    ...
    if (selected_type != '') {
        if (!seat.classList.contains(selected_type)) {
            ...
            const styleSheet = styleTag.sheet;
            const rule = Array.from(styleSheet.cssRules).find(
                (r) => r.selectorText === `.seat.${selected_type}`
            );
