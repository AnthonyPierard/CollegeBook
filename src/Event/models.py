...
class Event(models.Model):
    ...
    name = models.CharField("Nom de l'événement", max_length=200, unique=True,
                            validators=[RegexValidator(r'^[a-zA-Z0-9À-ÿ ]+$')])
...
class Representation(models.Model):
    ...
    remaining_seats = models.JSONField("Informations de la salle", blank=True,
                                       null=True)  # TODO pour tester aussi il faudra retirer blank et null
    state = models.CharField("Representation active ou archivée", choices=STATES, default='DRF', max_length=3)
    event = models.ForeignKey(Event, on_delete=models.CASCADE)
...
def trigger_representation_at_same_time(sender, instance, *args, **kwargs):
    all_rep_same_day = Representation.objects.filter(~Q(state="ARC"), date__day=instance.date.day).exclude(
        pk=instance.id)  # on recupere toutes les representations du meme jour sauf lui meme en cas de modif
    ...
    i_date = datetime.combine(instance.date, instance.date.time()).astimezone(
        pytz.utc)  # on transforme la date en UTC+0
    ...
    instance_time_end = instance_datetime_date + timedelta(hours=instance_duration.hour,
                                                           # on cree l heure de fin pour les tests
                                                           minutes=instance_duration.minute,
                                                           seconds=instance_duration.second)
    for rep in all_rep_same_day:  # on boucle sur toutes les rep
        rep_datetime_date = datetime.combine(rep.date, rep.date.time(), tzinfo=ZoneInfo(
            TIME_ZONE))  # on met la date de rep au meme format que celle de instance
        rep_duration = Event.objects.get(pk=rep.event.id).duration  # on recupere sa duree
        rep_time_end = rep_datetime_date + timedelta(hours=rep_duration.hour, minutes=rep_duration.minute,
                                                     # on cree la date de fin de rep pour les tests
