...
def event_details(request, even_id):
    ...
    representations = Representation.objects.filter(
        state="ACT", date__gte=timezone.now(), event=event.id
    )
    return render(
        request,
        "event_details.html",
        {"event": event, "representations": representations},
    )
...
def event_creation(request):
    if request.method == "POST":
        ...
        return render(
            request,
            "event_creation.html",
            {"form": form, "configurations": configurations},
        )
...
def delete_representation(request, representation_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if choice == "1":
                ...
                for reservation in reservations:
                    mail_content = (
                        "La representation de l'evenement %s,  le %s à malheureusement été annulée, vous pouvez contacter l'adresse @ pour plus d'information "
                        % (
                            reservation.representation.event.name,
                            reservation.representation.date,
                        )
                    )
                    ...
                    send_mail(
                        "Reservation Ticket %s" % reservation.representation.event.name,
                        "Vous avez reserver un Ticket pour la representation de %s"
                        % reservation.representation.event.name,
        ...
        return render(
            request, "delete_representation.html", {"form": form, "event": event}
        )
...
def event_update(request, event_id):
    if request.method == "POST":
        ...
        return render(
            request, "event_modification.html", {"form": form, "event": event}
        )
...
def publish_event(request, event_id):
    ...
    stripe.Product.create(
        ...
        default_price_data={"unit_amount": int(drink_price * 100), "currency": "eur"},
    ...
    stripe.Product.create(
        ...
        default_price_data={"unit_amount": int(food_price * 100), "currency": "eur"},
    ...
    if not path.joinpath("static/json/" + unidecode(event.name)).exists():
        path.joinpath("static/json/" + unidecode(event.name)).mkdir(
    ...
    applies_to = applies_to = [
        str(stripe_id_creation(element.type, unidecode(event.name)))
        for element in list(Price.objects.filter(event_id=event.id))
        + list(Place.objects.filter(configuration_id=event.configuration_id))
    ]
