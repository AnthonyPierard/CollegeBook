...
def check_event_is_archived():
    ...
    for current in all_event_not_archived:
        if (
            Representation.objects.filter(~Q(state="ARC"), event=current.id).count()
            == 0
        ):
            current.state = "ARC"
            current.save()
...
def send_remainders_mail():
    ...
    representations = Representation.objects.filter(
        date__year=end_date.year, date__month=end_date.month, date__day=end_date.day
    )
    for representationIT in representations:
        ...
        for reservation in reservations:
            reservation.email
            mail_content = (
                "N'oubliez pas ! La representation de %s est prévue le %s "
                % (
...
def send_thanks_mail():
    ...
    for representationIT in representations:
        ...
        for reservation in reservations:
            mail_content = (
                "Nous vous remerciont chaleureusement d'avoir été présent à la representation de %s,  le %s "
                % (
                    reservation.representation.event.name,
                    reservation.representation.date,
                )
