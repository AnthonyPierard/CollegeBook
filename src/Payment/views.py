...
class CreateCheckoutSessionView(View):
    def post(self, request, *args, **kwargs):
        ...
        line_items = [{'price': stripe.Product.retrieve(stripe_id_creation(key, event_name))["default_price"],
                       'quantity': tickets_quantity[key]} for key in tickets_quantity.keys()]
...
def landing(request, representation_id):
    ...
    seating_list = [{"type": ticket.type.type, "place": ticket.seat_number, "price": ticket.type.price} for ticket in
                    seating_tickets]
    ...
    return render(request, 'landing.html',
                  {"reservation": reservation, "standing_seats": standing_list, "seating_seats": seating_list,
                   "price": price})
...
def webhook(request):
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        for ticket in seating_tickets:
            ...
            pdf = create_ticket_pdf(pdf, trigramme, code, reservation.first_name, reservation.last_name,
                                    reservation.representation.event.name, reservation.representation.date)

        for ticket in standing_tickets:
            code = ticket.code
            pdf = create_ticket_pdf(pdf, "Debout", code, reservation.first_name, reservation.last_name,
                                    reservation.representation.event.name, reservation.representation.date)

        for ticket in food_tickets:
            code = ticket.code
            pdf = create_ticket_pdf(pdf, "Nourriture", code, reservation.first_name, reservation.last_name,
                                    reservation.representation.event.name, reservation.representation.date)

        for ticket in drink_tickets:
            code = ticket.code
            pdf = create_ticket_pdf(pdf, "Boisson", code, reservation.first_name, reservation.last_name,
                                    reservation.representation.event.name, reservation.representation.date)
