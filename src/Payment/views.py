...
class CreateCheckoutSessionView(View):
    def post(self, request, *args, **kwargs):
        ...
        line_items = [
            {
                "price": stripe.Product.retrieve(stripe_id_creation(key, event_name))[
                    "default_price"
                ],
                "quantity": tickets_quantity[key],
            }
            for key in tickets_quantity.keys()
        ]
        if reservation.drink_number > 0:
            line_items += [
                {
                    "price": stripe.Product.retrieve(
                        stripe_id_creation("boisson", event_name)
                    )["default_price"],
        ...
        if reservation.food_number > 0:
            line_items += [
                {
                    "price": stripe.Product.retrieve(
                        stripe_id_creation("nourriture", event_name)
                    )["default_price"],
        ...
        if (
            checkout_session_id == ""
            or stripe.checkout.Session.retrieve(checkout_session_id).status == "expired"
        ):
...
def landing(request, representation_id):
    ...
    standing_list = [
        {"type": ticket.type.type, "price": ticket.type.price}
    ...
    drink_price = Price.objects.get(
        event_id=reservation.representation.event.id, type="Boisson"
    ).price
    food_price = Price.objects.get(
        event_id=reservation.representation.event.id, type="Nourriture"
    ).price
    price = 0
    for seat in standing_list + seating_list:
        price += seat["price"]
    price += (
        reservation.food_number * food_price + reservation.drink_number * drink_price
    )
    ...
    return render(
        ...
        "landing.html",
        {
            ...
            "standing_seats": standing_list,
            "seating_seats": seating_list,
            "price": price,
        },
    )
...
def webhook(request):
    ...
    if event["type"] == "checkout.session.completed":
        ...
        selected_seats = [
            ticket.seat_number
            for ticket in Ticket.objects.filter(reservation_id=reservation.id)
        ]
